...
**Update, lane T.** All four are now implemented and none is live-verified.
`puerts_material_build` and `puerts_texture_import` are lane T's; the graph
builder is MUTATING, not read-only, and the reasoning that reverses lane I's
read-only verdict is finding 0s in `docs/CAPABILITY_FINDINGS.md`.
`puerts_texture_import` GENERATES a solid or checker texture and refuses
`source_file` by name: importing an arbitrary path needs an allowed import root
this bridge does not define. Every C++ file behind these two is uncompiled.
...
| 1 | `puerts_material_build` | NEW | lane T (settled, C++ uncompiled) | materials |
...
| 1 | `puerts_sequence_render` | NEW | lane Z, shipped as `puerts_sequence_render_start` on the job API (implemented, C++ uncompiled) | cinematics |
| 1 | `puerts_texture_import` | NEW | lane T (settled, C++ uncompiled, generates rather than imports) | materials |
