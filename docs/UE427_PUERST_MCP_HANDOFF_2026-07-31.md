...
| Editor shutdown | Fixed and verified 2026-08-01 | Five consecutive `bridge-idle` closes at 4.1 s each, zero locked plugin DLLs, no surviving advertisement, build ok after every one; `read-only` and `bt-smoke` likewise; reports in `reports/shutdown-acceptance-*.json` | `MCPBridgePIEAgent` removed its core ticker and its `GLog` output device only in `ShutdownModule`, which runs from `UnloadModulesAtShutdown` (`LaunchEngineLoop.cpp:4294`) after `StaticExit` has destroyed every UObject, leaving `GLog` holding a freed log sink. The editor stopped on the line `StaticExit` itself emits and never finished exiting, keeping the plugin DLLs locked. NOT PuerTS: `puerts-idle` never creates an `FJsEnv` and reproduced it, and the script environment releases in 0.003 s | Release moved to `FCoreDelegates::OnEnginePreExit` (`UnrealEngine.cpp:1878`) in `MCPBridgePIEAgent` and `MCPBridgePuerTS`; `pipe.txt` retracted on shutdown. Regression harness: `Scripts/editor-shutdown-acceptance.ps1`. See docs/CAPABILITY_FINDINGS.md defect 0f |
...
| Multi-project reuse | Proven with two editors open 2026-08-02 | Both targets carry an install manifest and pass `npm run install:check`. `Scripts/session-isolation-acceptance.mjs` ran with `BridgeInstallTest` and `Tests\UE427PuerTSMCP` open simultaneously: distinct session ids, pids, pipes and nonces; 12 interleaved read-only requests with zero crossed replies; probe actors confined to their own worlds; forged nonce and stale advertisement both refused; closing one editor left the other serving without the closed one falling through to it | Discovery no longer has any fallback, so a client with no advertised session refuses instead of connecting to whatever owns the default pipe name. That is a deliberate behaviour change: `MCP_PUERTS_PIPE` still selects a pipe but no longer decides identity | keep both targets synchronised; findings 0l has the detail |
...
- Editor teardown: `MCPBridgePIEAgent` and `MCPBridgePuerTS` release from `FCoreDelegates::OnEnginePreExit`, not from `ShutdownModule`. `ShutdownModule` runs after `StaticExit` has destroyed every UObject, so anything registered with `GLog`, the core ticker, or the root set must be unregistered before then. Doing it at module-unload time is what produced the unkillable editors; do not move it back, and follow the same pattern in any new module that registers one of those. The editor log carries `MCPBridge lifecycle:` lines for startup, shutdown begin with its trigger, script-environment release, PIE agent release and total duration. If a close hangs, read those first: the last one reached names the stage. `Scripts/editor-shutdown-acceptance.ps1` is the regression test.
