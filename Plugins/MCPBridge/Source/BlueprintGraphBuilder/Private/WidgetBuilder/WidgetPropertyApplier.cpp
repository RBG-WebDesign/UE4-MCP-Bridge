...
static FLinearColor ParseColorObject(const TSharedPtr<FJsonObject>& Obj, float DefaultA = 1.0f)
{
    return FLinearColor(
        ...
        Obj->HasField(TEXT("a")) ? static_cast<float>(Obj->GetNumberField(TEXT("a"))) : DefaultA
    );
}

static FMargin ParseMarginObject(const TSharedPtr<FJsonObject>& Obj)
{
    return FMargin(
        Obj->HasField(TEXT("left")) ? static_cast<float>(Obj->GetNumberField(TEXT("left"))) : 0.0f,
        Obj->HasField(TEXT("top")) ? static_cast<float>(Obj->GetNumberField(TEXT("top"))) : 0.0f,
        Obj->HasField(TEXT("right")) ? static_cast<float>(Obj->GetNumberField(TEXT("right"))) : 0.0f,
        Obj->HasField(TEXT("bottom")) ? static_cast<float>(Obj->GetNumberField(TEXT("bottom"))) : 0.0f
    );
}
    ...
    if (TypeName == TEXT("TextBlock"))
    {
        ...
        if (TextBlock)
        {
            ...
            if (ColorVal && (*ColorVal)->Type == EJson::Object)
            {
                ...
                FLinearColor Color = ParseColorObject(ColorObj);
                TextBlock->SetColorAndOpacity(FSlateColor(Color));
            }

            const TSharedPtr<FJsonValue>* FontSizeVal = Properties.Find(TEXT("fontSize"));
            if (FontSizeVal && (*FontSizeVal)->Type == EJson::Number)
            {
                FSlateFontInfo Font = TextBlock->Font;
                Font.Size = static_cast<int32>((*FontSizeVal)->AsNumber());
                TextBlock->SetFont(Font);
            }

            const TSharedPtr<FJsonValue>* AutoWrapVal = Properties.Find(TEXT("autoWrapText"));
            if (AutoWrapVal && (*AutoWrapVal)->Type == EJson::Boolean)
            {
                TextBlock->SetAutoWrapText((*AutoWrapVal)->AsBool());
            }

            const TSharedPtr<FJsonValue>* ShadowOffsetVal = Properties.Find(TEXT("shadowOffset"));
            if (ShadowOffsetVal && (*ShadowOffsetVal)->Type == EJson::Object)
            {
                const TSharedPtr<FJsonObject>& Obj = (*ShadowOffsetVal)->AsObject();
                TextBlock->SetShadowOffset(FVector2D(
                    Obj->HasField(TEXT("x")) ? static_cast<float>(Obj->GetNumberField(TEXT("x"))) : 0.0f,
                    Obj->HasField(TEXT("y")) ? static_cast<float>(Obj->GetNumberField(TEXT("y"))) : 0.0f
                ));
            }

            const TSharedPtr<FJsonValue>* ShadowColorVal = Properties.Find(TEXT("shadowColor"));
            if (ShadowColorVal && (*ShadowColorVal)->Type == EJson::Object)
            {
                TextBlock->SetShadowColorAndOpacity(ParseColorObject((*ShadowColorVal)->AsObject(), 0.65f));
            }
        }
    }
    else if (TypeName == TEXT("Border"))
    {
        ...
        if (Border)
        {
            ...
            if (ColorVal && (*ColorVal)->Type == EJson::Object)
            {
                Border->SetBrushColor(ParseColorObject((*ColorVal)->AsObject()));
            }

            const TSharedPtr<FJsonValue>* PaddingVal = Properties.Find(TEXT("padding"));
            if (PaddingVal && (*PaddingVal)->Type == EJson::Object)
            {
                Border->SetPadding(ParseMarginObject((*PaddingVal)->AsObject()));
            }
        }
    }
