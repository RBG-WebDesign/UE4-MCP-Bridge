...
//           (.gitignore). CI proves the missing-bundle refusal; a machine with
//           a bundle also proves staged-runtime integrity and the artefact.
//
// The packager must refuse to build a release out of a tree that would produce
...
try {
  ...
  console.log("-- 1. it refuses to package without the pinned bundle");
  ...
  console.log("\n-- 2. it refuses to package a plugin whose PuerTS lane is empty");
  if (!haveBundle) {
    skip("the staged-runtime gate requires a valid pinned bundle to reach it");
  } else {
    // Content/JavaScript/puerts is generated by npm run build from the pinned
    // bundle and is not in Git. Shipping without it produces a plugin that
    // loads and then has nothing to execute, which used to happen silently.
