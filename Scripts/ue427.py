...
def quarantined_tools() -> Dict[str, str]:
    """Tools the bridge implements but refuses to run, read from the generated
    inventory rather than from a list kept here. docs/TOOL_INVENTORY.json is
    produced by Scripts/generate-tool-inventory.mjs from the built server, so
    this stays in step with the code without a third copy of the names."""
    path = os.path.join(REPO_ROOT, "docs", "TOOL_INVENTORY.json")
    if not os.path.exists(path):
        return {}
    with open(path, "r", encoding="utf-8") as handle:
        inventory = json.load(handle)
    return {
        tool["name"]: tool.get("notes") or "quarantined"
        for tool in inventory.get("tools", [])
        if tool.get("migration_state") == "quarantined"
    }
...
def generate_catalog() -> str:
    ...
    # A quarantined tool is not offered under its safety class, because the
    # class answers "how careful should I be" and the answer here is "you
    # cannot call this". Listed separately so a reader learns it exists and why
    # it is refused, rather than trying it and reading a failure.
    quarantined = quarantined_tools()
    ...
    lines = [
        ...
        "",
        "Total: %d callable tools." % (len(classification) - len(quarantined)),
        "",
    ]
    if quarantined:
        ...
        lines.append("")
        lines.append(
            "Implemented, withheld from tool discovery, and refused by the runtime "
            "with `error_code: capability_quarantined`. Do not reach for these; "
            "`puerts_diagnostic` reports the same list under `capabilities.unavailable`.")
        lines.append("")
        for name in sorted(quarantined):
            lines.append("- `%s` - %s" % (name, quarantined[name]))
        lines.append("")
