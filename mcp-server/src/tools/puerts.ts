...
const commandTimeouts: Readonly<Record<string, number>> = {
  ...
  // A level of a few thousand actors is hashed three times per batch (plan,
  // verify, and once more if a rollback has to prove itself), and each spawn
  // runs the editor's own AddActor path. 60s is the member patch's ceiling and
  // is the right order of magnitude here.
